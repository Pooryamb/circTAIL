...
for i in range(NumOfReads):
    if ("C" in Lines[2*i + 1].strip()) or ("G" in Lines[2*i + 1].strip()):
...
df_Aln = pd.read_csv("../intermediates/L_alignment_pruned.txt", sep="\t", header=None)

ReadsFile = glob.glob("../input/*_renamed.fasta")[0]
SeqDict = FileContet2Dict(open(ReadsFile).read())

temp = open(glob.glob("../input/*_L.fa")[0]).readlines()[1].strip()
...
for ID in sorted_IDs:
    L_sde = df_Aln_sel[df_Aln_sel[0] == ID]  
    LINES.append("The alignment for read "+ str(ID) +" starts\n\n")
    qstart = L_sde[1].to_list()[0]
    qend = L_sde[2].to_list()[0]
    sstart = L_sde[3].to_list()[0]
    send = L_sde[4].to_list()[0]
    qseq_aln = L_sde[8].to_list()[0]
    sseq_aln = L_sde[9].to_list()[0]
    qseq = SeqDict[str(ID)]
    ...
    LINES = LINES + [L0, L1, L2, L3,"\n"]


#R_sde = df_Aln_R_sel[df_Aln_R_sel[0] == ID]
VisualizedLeftSide = open("../output/VisualizedL_side_CG_ContainingReads.txt",'w')
